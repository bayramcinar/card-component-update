...
function Card({ image, name, status,category, starNumber, job, videoNumber, callNumber, language, skills,showedSkillsNumber, price ,apointmentDate,commentNumber, minSessionTime}) {
  const renderStars = () => {
    const stars = [];
  ...
  return (
    <div>
      <div className="generalCard bg-white border-2 w-80 text-center ml-5 rounded-xl h-full justify-between flex flex-col px-4 py-2 min-w-xs max-w-md">
        <div className='favAndShareArea ml-auto'>
          <div className="favAndShareButtons">
             <button className='fav text-favShare text-xs'><i class="fa-regular fa-heart"></i></button>
             <button className='share text-favShare text-sm ml-3'><i class="fa-solid fa-arrow-up-from-bracket"></i></button>
          </div>
        </div>
        <div className='flex h-full flex-col justify-between'>
          ...
          <div className='relative imageArea w-full flex flex-col items-center justify-center'>
            ...
            {status !== undefined && (
              <span className={`status bg-lightBlue ${status === 1 ? 'text-blueOne' : 'text-redOne'} px-3 py-1 rounded-2xl text-sm border-${status === 1 ? 'blueOne' : 'redOne'} border relative bottom-3.5`}>
          ...
          <div className='viewProfileArea mt-1'>
            <button className=' font-semibold border-b-2 p-2.5'>Profili İncele<i className="fa-solid fa-arrow-right"></i></button>
          </div>
        </div>
        <div className='priceArea flex text-center justify-center m-3 mt-1'>
          <h4 className='font-semibold text-lg'>₺{price}/Seans</h4>
          <h4 className='ml-2 text-sm pt-1'> {`(Minimum ${minSessionTime} dakika)`}</h4>
        </div>
        <div className='appointmentButtonArea'>
          {status === 1 ? (<button className='bg-appointmentColor tracking-wider font-medium text-white w-72 py-2 rounded-3xl text-sm'>Hemen Randevu Al</button>):
            <button className='bg-appointmentColor font-medium tracking-wider text-white w-72 py-2 rounded-3xl text-sm'>Randevu Al</button>
          }
          <button className='bg-grayButton text-white font-medium tracking-wider w-72 py-2 rounded-3xl text-sm mt-2'>Mesaj Gönder</button>
        </div>
        <div className="infoTextArea">
          {status !== undefined && (
            <h5 className={`${status === 1 ? 'text-greenText' : 'text-redText'} text-xs font-medium mt-2`}>
              {status === 1 ? `*${name} şuan musait, randevu al butonuna tıklayarak hemen hizmet alabilirsiniz.` : `*${name} şuan musait değil, en yakın seansı: ${apointmentDate}`}
            </h5>
          )}
        </div>
